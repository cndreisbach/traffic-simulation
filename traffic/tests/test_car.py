...
def test_car_will_move():
    ...
    assert move(a_car).location == 30


def test_car_is_too_close():
    car1 = make_car(location=0, speed=30)
    car2 = make_car(location=20, speed=10)

    assert too_close(car1, car2)


def test_cars_cannot_pass():
    car1 = make_car(location=25, speed=30)
    car2 = make_car(location=20, speed=10)
